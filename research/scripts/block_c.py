...
DEFAULT_LEVELS = {ATTRACT: [1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0],
...
def calibrate_cached(world_name, topo, plans_b, gt_b, seed, n_persons, refit=False) -> dict:
    ...
    path = os.path.join(CALIB_DIR, f"C_{world_name}_seed{seed}_n{n_persons}_log.json")
...
def model_specs(scenario, alpha_cal, weights) -> List[tuple]:
    ...
    for s in GEN_MODELS:
        specs += [(s, "informed", "shock", alpha_cal),
                  (s, "blind", "base", alpha_cal),
                  (s, "no_attr", no_attr_view, 0.0)]
    for s in argmin:
        for w in weights:
...
def evaluate_cell(scenario, topo_base, base_sizes, ld, solver, condition, view, alpha, seed, cal):
    ...
    rdf, valid = predict(topo_model, sizes_field, ld["plans_in"], solver, alpha, seed, cal, cap=cap)
    ...
    row = {"scenario": scenario, "solver": solver, "condition": condition, "view": view,
           ...
           "dist_w_m": dist_w, "pot_decile_tv": pot_tv, "combined_cost": combined_cost,
...
def run_scenario(world, world_name, scenario, topo, base_sizes, district_mask, cal,
    ...
    print(f"--- scenario={scenario}  base_{LEVEL_NAME[scenario]}_outcome={base_main:.4g} ---", flush=True)
    ...
    for lv in levels:
        if scenario == ATTRACT:
            sizes_shock = base_sizes.copy(); sizes_shock[district_mask] *= lv
            topo_div = None
        ...
        level_data[lv] = {"level": lv, "plans_in": plans_in, "gt": gt_cf, "free_ids": free_ids,
                          "scored": _scored_legs(plans_in, gt_cf),
                          "district_ids": district_ids, "true_main": true_main, "base_main": base_main,
                          "true_dists": true_d[np.isfinite(true_d)], "sizes_shock": sizes_shock,
                          "topo_div": topo_div}
    ...
    rows += [{"scenario": scenario, "solver": "true", "condition": "true", "view": "true",
              ...
              "true_main": level_data[lv]["true_main"], "base_main": base_main, "valid": True}
             for lv in levels]
    df = pd.DataFrame(rows)
    df["world"] = world_name; df["alpha_cal"] = cal["alpha"]
    os.makedirs(out_dir, exist_ok=True)
    df.to_csv(os.path.join(out_dir, f"prognosis_{scenario}_{world_name}.csv"), index=False)
    if raws:
        raw_df = pd.concat(raws, ignore_index=True); raw_df["world"] = world_name
        raw_df.to_parquet(os.path.join(out_dir, f"raw_legs_{scenario}_{world_name}.parquet"), index=False)
...
def run_true_curves(world_name, scenarios, n_persons, seed, out_dir):
    ...
    district_ids = set(topo.loc_ids[_district_mask(world_name, topo)])
    rows = []
    for sc in scenarios:
        levels = DEFAULT_LEVELS[sc] if _LEVELS_OVR is None else _LEVELS_OVR.get(sc, DEFAULT_LEVELS[sc])
        for lv in levels:
            if sc == ATTRACT:
                sizes = base_sizes.copy(); sizes[_district_mask(world_name, topo)] *= lv
...
def main(argv=None):
    global _WEIGHTS, _LEVELS_OVR, _POTK, ACCESS_LEVER
