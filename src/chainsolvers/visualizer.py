...
class Visualizer:
    def __init__(
        self,
        savedir: str,
        map_prefix: str = "anchorchain",
        *,
        source_crs: Any = 25832,
        target_crs: Any = 4326,
    ):
        """
        savedir: directory where HTML maps are saved
        map_prefix: file name prefix for saved maps (e.g., 'anchorchain', 'carla')
        source_crs: CRS of the input coordinates (EPSG code or any pyproj CRS spec);
            defaults to EPSG:25832 (UTM zone 32N).
        target_crs: CRS used for the folium map; defaults to EPSG:4326 (WGS84),
