...
class Carla:
    ...
    **params : dict
    ...
    def solve(
        ...
        if self.visualizer:
            if len(plans) != 1:
                raise ValueError("Visualizer can only handle one person-tour at a time.")
    ...
    def solve_segment(self, segment: Segment, parent_node=None) -> Tuple[Segment, float]:
        ...
        # base: two legs
            ...
            # get_best_circle_intersection_locations handles the degenerate (non-intersecting) case
            # internally now: it unions the projected point's nearest facilities with overlapping-ring
            # candidates, monotonically (see locations.get_circle_intersection_candidates).
