...
class Locations:
    ...
    def query_closest(
        ...
        # KD-tree query: idx can be a python int, scalar, (k,), (m,), or (m,k)
        _, idx = self.trees[act_type].query(loc, k=k)
        idx = np.asarray(idx)  # coerce python int (single point, k=1) to a 0-d array
        ...
        while True:
            ...
            if cand is not None:
                ...
                if ids.size >= min_candidates:
                    ...
                    return ids, coords, pots
                # Track the largest set seen; expansion may not reach min_candidates if the
                # type has fewer locations than requested (e.g. a rare activity type).
                if best is None or ids.size > best[0].shape[0]:
                    best, stale = (ids, coords, pots), 0
                else:
                    stale += 1
            else:
                stale += 1

            if stale >= 3 and best is not None:
                return best
    ...
    def get_overlapping_rings_candidates(
        ...
        while True:
            ...
            if cand is not None:
                ...
                # Track the largest set seen; detect when expansion stops helping.
                if best is None or m > best[0].shape[0]:
                    best, stale = cand, 0
                else:
                    stale += 1
            else:
            ...
            # Expanding the rings no longer adds candidates (e.g. min_candidates exceeds
            # the reachable count once self-coinciding anchors are excluded): return best.
            if stale >= 3 and best is not None:
                return best, i
    ...
    def get_best_overlap_candidates(
        ...
        Two distance-deviation arrays are computed, on purpose:

        - ``temp_deviations`` — an *abstract* deviation that measures how far each
          candidate sits outside the [r_min, r_max] envelope on either side. This
          reflects the full multi-leg sub-chain bounds and is only used *locally*
          to rank/select candidates at this recursion level, then discarded.
        - ``permanent_deviations`` — the *concrete* deviation, populated only for a
          side whose sub-chain is a single leg (i.e. an actual leaf placement with a
          known target distance). This is the value returned to the caller. CARLA
          sums the returned scores as it recurses, so counting only leaf-leg
          deviations here avoids double-counting the same geometric error once the
          recursion descends and re-measures that leg directly.

        Both arrays are scored with the injected `scorer`; selection uses the temp
        scores (richer local signal) while the returned scores come from the
        permanent deviations (additive across levels).
        """
