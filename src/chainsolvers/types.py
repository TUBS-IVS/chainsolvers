...
from typing import Tuple, NamedTuple, Mapping, Any, Union, Sequence, Optional, Protocol, runtime_checkable, Set
...
class Leg(NamedTuple):
    ...
    connected_legs: Tuple[str, ...] | None = None
...
class PlanColumns:
    ...
    connected_legs: str | None = "connected_legs"
...
# A solver's ``solve(...)`` may return either an internal plans structure
# (frozendict / dict, normalized to SegmentedPlans by the runner) or a DataFrame.
SolverResult = Union["frozendict", dict, Any]


@runtime_checkable
class Solver(Protocol):
    """
    Structural contract every solver must satisfy (enforced at instantiation in
    ``run._instantiate_solver``).

    - ``wanted_format``: one of ``"segmented_plans"``, ``"households"``, ``"df"``;
      selects how the runner shapes input and which keyword (``plans=`` /
      ``households=`` / ``df=``) ``solve`` is called with.
    - ``required_leg_fields()``: the ``Leg`` field names the solver consumes; the
      runner maps these to the DataFrame columns it validates on input.
    - ``solve(...)``: returns an internal plans structure or a DataFrame.
    """
